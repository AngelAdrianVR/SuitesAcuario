<template>
    ...
    <div v-if="showMobileMenu"
        ...
        <button
            ...
            @click="$inertia.visit('/')">Inicio</button>
        <button
            class="mx-2 rounded-sm px-1 py-px hover:text-primary text-white transition-colors ease-linear duration-200"
            @click="$inertia.visit('/')">Nosotros</button>
        <button
            class="mx-2 rounded-sm px-1 py-px hover:text-primary text-white transition-colors ease-linear duration-200"
            @click="$inertia.visit('/')">Suites</button>
        <button
            class="mx-2 rounded-sm px-1 py-px hover:text-primary text-white transition-colors ease-linear duration-200"
            @click="$inertia.visit('/')">Contacto</button>
    </div>
    ...
    <nav :class="['navbar', { 'fixed-navbar': isNavbarFixed }]"
        ...
        <!-- info de contacto  -->
        <div
            class="absolute top-0 right-0 bg-white border-b-8 border-black py-2 pl-5 pr-36 lg:flex items-center space-x-5 hidden ">
        ...
        <div class="lg:flex justify-center mr-12 hidden">
            ...
            <button
                class="mx-2 rounded-sm px-1 py-px hover:text-primary text-white transition-colors ease-linear duration-200"
                @click="$inertia.visit('/')">Contacto</button>
            <a href="https://api.whatsapp.com/send?phone=526691178743&text=Hola!%20vi%20tu%20página%20,%20me%20interesa%20su%20servicio!"
    ...
    <main @click="showMobileMenu = false" class="lg:py-10 lg:px-32 px-3 mt-24">
        ...
        <div class="lg:grid grid-cols-2 gap-9 mt-5 relative space-y-3 lg:space-y-0">
            ...
            <div class="grid grid-cols-2 gap-7 sm:mt-7 lg:mt-0">
                <figure v-if="suites[suite - 1].images?.length > 1" :class="{ 'active': image2 }"
                    class="w-full fade-in">
                    <img class="object-cover rounded-xl w-full h-[286px]" :src="suites[suite - 1].images[1]" alt="">
                </figure>
                <figure v-if="suites[suite - 1].images?.length > 2" :class="{ 'active': image3 }"
                    class="w-full fade-in-right">
                    <img class="object-cover rounded-xl w-full h-[286px]" :src="suites[suite - 1].images[2]" alt="">
                </figure>
                <figure v-if="suites[suite - 1].images?.length > 3" :class="{ 'active': image4 }"
                    class="w-full fade-in">
                    <img class="object-cover rounded-xl w-full h-[286px]" :src="suites[suite - 1].images[3]" alt="">
                </figure>
                <figure v-if="suites[suite - 1].images?.length > 4" :class="{ 'active': image5 }"
                    class="w-full fade-in-right ">
                    <img class="object-cover rounded-xl w-full h-[286px]" :src="suites[suite - 1].images[4]" alt="">
                </figure>
            </div>
        ...
        <div class="flex justify-start items-center space-x-2 lg:space-x-7 mt-20 mb-12">
            ...
            <h2 class="text-lg lg:text-xl font-bold text-center text">HABITACIÓN <span
                    class="text-primary uppercase ml-1">{{ suites[suite - 1].name }}</span> </h2>
            <div class="border-b-4 border-primary w-40 mt-[2px]"></div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-x-7 gap-y-3 my-7">
        ...
        <div class="flex justify-end">
            <a class="!px-16 !text-base hidden lg:block text-center py-2 bg-primary border border-transparent rounded-full font-semibold text-white tracking-widest hover:bg-primary active:bg-primary active:scale-95 disabled:active:scale-100 disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-gray2 focus:outline-none focus:ring-0 transition ease-in-out duration-100"
                :href="'https://api.whatsapp.com/send?phone=6691178743&text=¡Hola!%20vi%20tu%20página,%20quiero%20más%20información%20sobre%20'+suites[suite - 1].name">
                Enviar mensaje
            </a>
        </div>
    </main>
